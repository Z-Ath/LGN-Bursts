...
class FERunConfig:
    ...
    n_splits: int = 5
    random_state: int = 42
    results_root: str = "./results/fe_randgt"
...
def _sample_random_ground_truth_from_other_sessions(
    donor_targets: dict[str, np.ndarray],
    ...
    available_session_ids = [
        session_id
        for session_id, y in donor_targets.items()
        if session_id != exclude_session_id and np.asarray(y).ndim == 2 and y.shape[1] > 0
    ]
    if not available_session_ids:
        raise ValueError("No donor sessions available for random ground truth sampling.")

    y_rand = np.empty((n_samples, target_dim), dtype=np.float32)
    row_session_ids = rng.choice(available_session_ids, size=n_samples, replace=True)

    for row_idx, donor_session_id in enumerate(row_session_ids):
        donor_y = np.asarray(donor_targets[donor_session_id], dtype=np.float32)
        donor_n_samples, donor_target_dim = donor_y.shape
        sampled_row_idx = int(rng.integers(donor_n_samples))
        sampled_col_idx = rng.integers(donor_target_dim, size=target_dim)
        y_rand[row_idx] = donor_y[sampled_row_idx, sampled_col_idx]
...
def run_fe_xgb_pipeline(
    ...
    if control_config.familiar_mode not in allowed_modes:
        raise ValueError(
            f"Unsupported familiar_mode={control_config.familiar_mode!r} for embedding_xgboost_baseline. "
            f"Allowed modes: {sorted(allowed_modes)}"
        )
    if control_config.deviant_mode not in allowed_modes:
        raise ValueError(
            f"Unsupported deviant_mode={control_config.deviant_mode!r} for embedding_xgboost_baseline. "
            f"Allowed modes: {sorted(allowed_modes)}"
        )
    ...
    for session_id in session_ids:
        print(f"\n=== Session: {session_id} ===")
        X_raw = session_data_cache[session_id]["X_raw"]
        y = session_data_cache[session_id]["y"]
        meta = session_data_cache[session_id]["meta"]
        ...
        y_rand_gt = _sample_random_ground_truth_from_other_sessions(
            donor_targets={sid: payload["y"] for sid, payload in session_data_cache.items()},
        ...
        np.save(prediction_dir / f"{session_id}_y_true.npy", y_rand_gt)
        np.save(prediction_dir / f"{session_id}_y_true_original.npy", y)
        np.save(prediction_dir / f"{session_id}_y_pred_oof.npy", y_pred_oof)

        r2_per_neuron, pearson_per_neuron = _compute_prediction_metrics_per_dim(y_rand_gt, y_pred_oof)
        r2_per_trial, pearson_per_trial = _compute_prediction_metrics_per_trial(y_rand_gt, y_pred_oof)
    ...
    tag = f"{config.pred}_{'mean' if config.pred_mean else 'full'}_fe_randgt"
    ...
    print("\nSaved random_target_control outputs:")
    print(f"  {eval_path}")
...
if __name__ == "__main__":
    config = FERunConfig(
        ...
        n_splits=5,
        random_state=42,
        results_root="./results/fe_randgt",
        variance_threshold=0.95,
        max_components=30,
        familiar_mode="none",  # none | gaussian_trials
        deviant_mode="none",  # none | gaussian_trials
    )
