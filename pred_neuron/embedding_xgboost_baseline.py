...
class ViTEmbeddingFeatureEngineer:
    ...
    def _n_components(self, X_train):
        """Choose the PCA dimension, or use fixed_components when provided."""
...
class FERunConfig:
    ...
    n_splits: int = 5
    random_state: int = 42
    results_root: str = "./results/fe_v1"
...
def run_fe_xgb_pipeline(
    ...
    tag = f"{config.pred}_{'mean' if config.pred_mean else 'full'}_fe_v1"
    ...
    print("\nSaved embedding_xgboost_baseline outputs:")
    print(f"  {eval_path}")
