...
@dataclass
class FENeuronFeatConfig:
    pred: str = "burst_B"
    pred_mean: bool = False
    need_sup_feat: bool = False
    n_splits: int = 5
    random_state: int = 42
    results_root: str = "./results/fe_neuron_feat"
...
class MultiBlockEmbeddingFeatureEngineer:
    def __init__(
        self,
        d_img: int,
        variance_threshold: float = 0.95,
    ...
    def _split_raw(self, X) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        X_np = self._to_numpy(X)
        expected_min_dim = self.d_img * 2
        if X_np.shape[1] < expected_min_dim:
            raise ValueError(
                f"Expected at least {expected_min_dim} raw features for two embeddings, "
                f"got {X_np.shape[1]}."
            )
        familiar = X_np[:, : self.d_img]
        deviant = X_np[:, self.d_img : 2 * self.d_img]
        support = X_np[:, 2 * self.d_img :] if X_np.shape[1] > expected_min_dim else np.empty(
            (X_np.shape[0], 0),
            dtype=np.float32,
        )
        return familiar, deviant, support

    def _build_blocks(self, X) -> tuple[dict[str, np.ndarray], np.ndarray]:
        familiar, deviant, support = self._split_raw(X)
        diff = familiar - deviant
        hadamard = familiar * deviant
        abs_diff = np.abs(diff)
        summed = familiar + deviant
    ...
    def _n_components(self, X_train: np.ndarray) -> int:
        max_valid = min(X_train.shape[0] - 1, X_train.shape[1], self.max_components)
        if max_valid <= 0:
            raise ValueError(f"Not enough training samples for PCA: got shape {X_train.shape}")
    ...
    def fit_transform(self, X) -> np.ndarray:
        blocks, support = self._build_blocks(X)
        ...
        for group_name, block in blocks.items():
            n_components = self._n_components(block)
            pca = PCA(n_components=n_components)
            block_reduced = pca.fit_transform(block)
            self.pca_by_group[group_name] = pca
            self.group_dims_[group_name] = int(block_reduced.shape[1])
            self.group_slices_[group_name] = slice(start, start + block_reduced.shape[1])
            start += block_reduced.shape[1]
            transformed_blocks.append(block_reduced)

        if support.shape[1] > 0:
            self.group_dims_["support"] = int(support.shape[1])
            self.group_slices_["support"] = slice(start, start + support.shape[1])
            transformed_blocks.append(support)
            start += support.shape[1]

        features = np.concatenate(transformed_blocks, axis=1)
        features = self.scaler.fit_transform(features)

        dims_text = ", ".join(f"{name}:{self.group_dims_[name]}" for name in self.group_dims_)
        print(f"  [FE] block dims -> {dims_text}; total={features.shape[1]}")
        return features.astype(np.float32)

    def transform(self, X) -> np.ndarray:
        if not self.pca_by_group:
            raise AssertionError("Please call fit_transform before transform.")

        blocks, support = self._build_blocks(X)
        transformed_blocks = []

        for group_name, block in blocks.items():
            if group_name not in self.pca_by_group:
                raise KeyError(f"Missing fitted PCA for group '{group_name}'.")
            transformed_blocks.append(self.pca_by_group[group_name].transform(block))
    ...
    @property
    def feature_group_slices(self) -> dict[str, slice]:
        return dict(self.group_slices_)
...
def compute_oof_predictions_with_multiblock_fe(
    X_raw: np.ndarray,
    y: np.ndarray,
    d_img: int,
    xgb_params: Dict,
    ...
    for fold_idx, (tr_idx, val_idx) in enumerate(kf.split(X_raw), start=1):
        ...
        fe = MultiBlockEmbeddingFeatureEngineer(
            d_img=d_img,
...
def _build_control_config(config: FENeuronFeatConfig) -> ControlConfig:
    ...
    if control_config.familiar_mode not in allowed_modes:
        raise ValueError(
            f"Unsupported familiar_mode={control_config.familiar_mode!r} for multiblock_neuron_importance. "
            f"Allowed modes: {sorted(allowed_modes)}"
        )
    if control_config.deviant_mode not in allowed_modes:
        raise ValueError(
            f"Unsupported deviant_mode={control_config.deviant_mode!r} for multiblock_neuron_importance. "
            f"Allowed modes: {sorted(allowed_modes)}"
        )
...
def _plot_top_sessions(
    ...
    axes[0].set_title("Top Sessions Ranked by Filtered Pearson Mean")
...
def _export_paper_experiment_data(
    ...
    export_metadata = {
        "description": (
            "Paper-ready experiment package exported from multiblock_neuron_importance. "
...
def _sum_group_importance(mean_abs: np.ndarray, group_slices: dict[str, slice], group_name: str) -> float:
    if group_name not in group_slices:
        return 0.0
    group_slice = group_slices[group_name]
    return float(mean_abs[group_slice].sum())
...
def _compute_filtered_shap_rows(
    ...
    for session_info in selected_session_infos:
        ...
        X_feat = fe.fit_transform(X_raw)
        group_slices = fe.feature_group_slices

        print(f"\n=== SHAP for session: {session_id} / kept_neurons={len(keep_indices)} ===")
        for neuron_idx in keep_indices:
            model = XGBRegressor(**shap_params)
            model.fit(X_feat, y[:, neuron_idx], verbose=False)

            explainer = shap.TreeExplainer(model)
            shap_values = explainer.shap_values(X_feat)
            mean_abs = np.mean(np.abs(shap_values), axis=0)

            familiar_imp = _sum_group_importance(mean_abs, group_slices, "familiar")
            deviant_imp = _sum_group_importance(mean_abs, group_slices, "deviant")
            diff_imp = _sum_group_importance(mean_abs, group_slices, "diff")
            hadamard_imp = _sum_group_importance(mean_abs, group_slices, "hadamard")
            abs_diff_imp = _sum_group_importance(mean_abs, group_slices, "abs_diff")
            sum_imp = _sum_group_importance(mean_abs, group_slices, "sum")
            support_imp = _sum_group_importance(mean_abs, group_slices, "support")
            ...
            per_dim_rows.append(
                {
                    ...
                    "diff_importance": diff_imp,
                    "hadamard_importance": hadamard_imp,
                    "abs_diff_importance": abs_diff_imp,
                    "sum_importance": sum_imp,
                    "support_importance": support_imp,
                    "interaction_importance": diff_imp + hadamard_imp + abs_diff_imp + sum_imp,
...
def run_fe_neuron_feat_pipeline(
    session_ids=SESSION_IDS,
    config: Optional[FENeuronFeatConfig] = None,
    ...
    print("\n=== Running FE + multi-block feature engineering + neuron filter on all sessions ===")
    for session_id in session_ids:
        ...
        y_pred_oof = compute_oof_predictions_with_multiblock_fe(
            X_raw=X_raw,
            y=y,
            d_img=int(meta["img_feature_dim"]),
            xgb_params=xgb_params,
    ...
    session_tag = f"{config.pred}_{'mean' if config.pred_mean else 'full'}_fe_neuron_feat"
    ...
    _plot_top_sessions(df_eval=df_top_sessions, output_path=metric_plot_path, figure_dpi=config.figure_dpi)
    ...
    metadata = {
        ...
        "notes": {
            ...
            "feature_rule": "Within each CV fold, familiar, deviant, difference, Hadamard product, absolute difference, and sum embedding blocks are each reduced with PCA, then concatenated together with optional support features before XGBoost.",
            "shap_rule": "SHAP is computed on the engineered feature space for kept neurons in selected sessions, while the scatter plot still uses familiar and deviant block importances and stores interaction block importances separately.",
        },
        "files": {
    ...
    print("\nSaved multiblock_neuron_importance outputs:")
...
def export_paper_experiment_data(config: Optional[FENeuronFeatConfig] = None):
    if config is None:
        config = FENeuronFeatConfig()
...
def run_plot(config: Optional[FENeuronFeatConfig] = None):
    if config is None:
        config = FENeuronFeatConfig()
    ...
    print("Re-generated multiblock_neuron_importance plots:")
    print(f"  {metric_plot_path}")
    print(f"  {shap_plot_path}")
...
if __name__ == "__main__":
    # Engineered feature blocks used before XGBoost:
    # 1. familiar embedding -> PCA
    # 2. deviant embedding -> PCA
    # 3. familiar - deviant -> PCA
    # 4. familiar * deviant (Hadamard product) -> PCA
    # 5. abs(familiar - deviant) -> PCA
    # 6. familiar + deviant -> PCA
    # 7. optional supporting features are appended without PCA when need_sup_feat=True
    config = FENeuronFeatConfig(
        ...
        n_splits=5,
        random_state=42,
        results_root="./results/fe_neuron_feat",
